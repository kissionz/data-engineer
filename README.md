...
- `/compact`：立即生成结构化会话摘要，保留当前请求、Todo、验证证据与未完成调用；最后一次用户请求会作为原文锚点重新注入，底层事件日志仍保持完整。
...
对工作区外的普通文件路径，审批界面提供三种范围：仅当前调用、当前进程内递归
...
`Glob` 按文件名模式递归发现文件，例如 `**/*文件名*`；只有需要浏览目录结构时才用
`ListDirectory`（默认只列当前一层；明确需要目录树时，可通过 `max_depth` 调整到
2–10 层）。外部目录结果使用可直接传给后续工具的绝对路径。
未授权且模型未知的目录不会被自动扫描；首次访问仍需用户给出起始路径并完成授权。
...
HTTP MCP 会把协议、规范化 host 和端口绑定到配置的 endpoint。DNS 与目标 IP 使用和 `HttpFetch` 相同的公共地址检查；Bearer token 只会发送到配置 URL 的精确 origin，不会转发到其他 allowlist 主机。
...
官方 OpenAI endpoint 使用内置能力默认值；OpenAI-compatible endpoint 不会伪造 context window 或最大输出长度。兼容模型可在可信用户配置中显式声明：
